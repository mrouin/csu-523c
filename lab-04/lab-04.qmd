...
#| fig-align: "center"
#| out.width: "100%"
knitr::include_graphics('lab-04/img/05-flood-map.jpg')
```
...
#| fig-align: "center"
#| out.width: "60%"
knitr::include_graphics('lab-04/img/palo-flood-warning.png')
```
...
```{r, echo = FALSE}
knitr::include_graphics("lab-04/img/lsat8-bands.jpg")
```
...
raster_files <- list.files(path = "lab-04/data",
  pattern = "*.TIF$",
  full.names = TRUE,
  recursive = TRUE)
...
#this NDWI map shows areas with water
```
...
swi_flood  <- app(SWI,  function(x) ifelse(x < 0, 1, 0))
...
The NDWI map has a more refined view of of the water areas whereas the WRI, NDVI, and MNDWI maps are more generalized. The SWI map does not have flood mapping.
...
#the dimensions are 12192 rows x 5 columns where the columns are the bands and the rows are the raster pixels.
```
...
```{r}
k <- 3
kmeans_stack <- kmeans(stack_extracted, centers = k)
```
...
```{r}
values(kmeans_raster) <- kmeans_stack$cluster
plot(kmeans_raster,
     col = c("blue", "white"))
```
...
```{r}
flood_table <- table(values(stacked_flood[[1]]), values(kmeans_raster))
...
cell_area <- res(flood_stack)[1] * res(flood_stack)[1]
cell_area

area_flood_calc <- area_flood * cell_area
area_flood_calc
```
...
plot(flood_certainty, colo = blues9)
```
...
mapview(flood_certainty_clean, col.regions = blues9)

#Somwhow the map inverts the flood certainty values so the areas that are likely considered to be flood areas are shown as low certainty and the areas that are likely land are considered to be high flood areas. It is possible that some cells are not assign to numbers because they did not fall within the sclae for the flood certainty.
```
