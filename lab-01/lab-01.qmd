...
**a.** For each gauge, compute the **7-day rolling mean** of daily discharge. Use `zoo::rollmean()` with `fill = NA` and `align = "right"` so each value represents the average of the current and 6 preceding days. If you haven't see this function before, check out the documentation (`?rollmean`) and examples to understand how it works.
